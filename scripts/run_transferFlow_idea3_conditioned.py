...
from memflow.transfer_flow.transfer_flow_idea3_conditioned import TransferFlow_idea3_conditioning
...
def L2(model, batch):
    ...
    scaled_partons_lab_flat = torch.flatten(logScaled_partons, start_dim=1)   
    flow_prob = model.flow(scaled_partons_lab_flat).log_prob(conditioning_event)
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_idea3_conditioning(no_recoVars=config.input_shape.no_recoVars,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name, config.version, 'Idea 3', 'Conditioned'])
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    if torch.isnan(wrong_pt_batch_flow_pr).any() or torch.isnan(wrong_ptAndEta_batch_flow_pr).any():
                        print(f'validation_plots_nans_jets: wrong_pt = {torch.count_nonzero(torch.isnan(wrong_pt_batch_flow_pr))}     & wrong_pt_eta = {torch.count_nonzero(torch.isnan(wrong_ptAndEta_batch_flow_pr))}')
