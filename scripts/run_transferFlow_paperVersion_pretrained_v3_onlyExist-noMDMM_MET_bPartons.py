...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'bpartons', 'leptonMETFirst+newFlows+H/thad/tlep', 'noMDMM', '3DFlow', 'eta_removedFromMET', 'onlyExistJets', 'no-btag', f'min5Jets={min5Jets}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    pos_partons = torch.tensor([0,1,2,3,4,5,6,7], device=device, dtype=dtype).unsqueeze(dim=1) # higgs, t1, t2, ISR
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            print(logScaled_bpartons.shape)

            logScaled_partons = torch.cat((logScaled_partons[:,0:1], # H
                                        logScaled_bpartons[:,0:2],        # H1,H2
                                        logScaled_partons[:,1:2],  # thad
                                        logScaled_bpartons[:,2:3],        # thad1
                                        logScaled_partons[:,2:3],  # tlep
                                        logScaled_bpartons[:,3:4],        # tlep1
                                        logScaled_partons[:,3:4]), dim=1) # ISR
            ...
            logScaled_partons = attach_position(logScaled_partons[:,:,:3], pos_partons)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_v3_MET_bPartons_scaled_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
