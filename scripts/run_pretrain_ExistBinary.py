...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    modelName = f"{name_dir}/model_Classifier_ExistBinary_{config.name}_{config.version}.pt"
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name, config.version, 'hot-encoded+inTransformer', 'Classifier_notFlow', f'min6Jets_max8={easy_version}', 'only_exist_pt_eta_phi', 'jetsSortedbySpanet', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            loss_main = loss(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])
            ...
            loss_main_perObj = loss_eachObj(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects, 0])
            ...
            loss_per_pt = compute_loss_per_pt(loss_per_pt, loss_main_perObj, logScaled_reco_sortedBySpanet, mask_recoParticles, log_mean_reco, log_std_reco, config.transferFlow.no_max_objects, pt_bins=pt_bins)
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                loss_per_pt = compute_loss_per_pt(loss_per_pt, loss_main_perObj, logScaled_reco_sortedBySpanet, mask_recoParticles, log_mean_reco, log_std_reco, config.transferFlow.no_max_objects,
                        pt_bins=pt_bins)
                ...
                confidence_batch, accuracy_batch, noElems_bins_batch = check_calibration_batch(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0], calibration_bins)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Classifier_ExistBinary_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
