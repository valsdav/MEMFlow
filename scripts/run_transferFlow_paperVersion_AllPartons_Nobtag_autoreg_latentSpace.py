...
from memflow.transfer_flow.transfer_flow_paper_AllPartons_Nobtag_autoreg_latentSpace import TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace(no_recoVars=5, # exist + 3-mom + encoded_position,
                                 no_partonVars=4,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['No_btag', 'latentSpace', 'newScaledLogReco-phi', 'classifierOnlyNonExist', 'periodicPhi', 'leptonMETFirst+newFlows+H/thad/tlep', '1Dflow-autoreg', 'onlyExistJets', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_{randint(0, 1000)}")
    else:
        exp = None
    ...
    pos_partons = torch.tensor([pos for pos in range(12)], device=device, dtype=dtype).unsqueeze(dim=1) # higgs, t1, t2, ISR
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0 and e % 2 == 0:
                    ...
                    fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
