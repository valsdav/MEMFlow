...
def constrain_energy(higgs, thad, tlep, mean, std):
    ...
    return logE_higgs, logE_thad, logE_tlep

def gluon_mom(higgs, thad, tlep, mean, std):
    ...
    sum_px = -(regressed_higgs[:,0] + regressed_thad[:,0] + regressed_tlep[:,0])
    sum_py = -(regressed_higgs[:,1] + regressed_thad[:,1] + regressed_tlep[:,1])
    sum_pz = -(regressed_higgs[:,2] + regressed_thad[:,2] + regressed_tlep[:,2])
    sum_E = torch.sqrt(sum_px**2 + sum_py**2 + sum_pz**2)

    logsum_px = torch.sign(sum_px)*torch.log(1 + torch.abs(sum_px))
    logsum_py = torch.sign(sum_py)*torch.log(1 + torch.abs(sum_py))
    logsum_pz = torch.sign(sum_pz)*torch.log(1 + torch.abs(sum_pz))
    log_E = torch.log(1 + torch.abs(sum_E))

    gluon_logpx = (logsum_px - mean[1])/std[1]
    gluon_logpy = (logsum_py - mean[2])/std[2]
    gluon_logpz = (logsum_pz - mean[3])/std[3]
    gluon_logE = (log_E - mean[0])/std[0]

    return gluon_logE.unsqueeze(dim=1), gluon_logpx.unsqueeze(dim=1), \
            gluon_logpy.unsqueeze(dim=1), gluon_logpz.unsqueeze(dim=1)
...
def TrainingAndValidLoop(config, device, model, trainingLoader, validLoader, outputDir):
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data in enumerate(trainingLoader):
            ...
            if (len(out) != 3):
                print("ERROR: Gluon version must have 3 outputs")
                exit(0)
            ...
            gluon_logE, gluon_logpx, gluon_logpy, gluon_logpz = gluon_mom(higgs, thad, tlep, log_mean, log_std)

            logE_higgs, logE_thad, logE_tlep = constrain_energy(higgs, thad, tlep, log_mean, log_std)

            higgs = torch.concat((logE_higgs, higgs), dim=1)
            thad = torch.concat((logE_thad, thad), dim=1)
            tlep = torch.concat((logE_tlep, tlep), dim=1)
            ISR = torch.concat((gluon_logE, gluon_logpx, gluon_logpy, gluon_logpz), dim=1)
...
if __name__ == '__main__':
    ...
    print(f"Gluon version: preTraining finished succesfully! Version: {conf.version}")
