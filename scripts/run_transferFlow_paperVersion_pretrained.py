...
from memflow.transfer_flow.transfer_flow_paper_pretrained import TransferFlow_Paper_pretrained
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_pretrained(no_recoVars=5, # exist + 3-mom + encoded_position
                no_partonVars=4,
                ...
                freeze_classifier=config.DNN.freeze_weights,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', f'classifier_freezeWeights:{config.DNN.freeze_weights}', 'no-btag', f'min6Jets_max8={easy_version}', 'only_exist_pt_eta_phi', 'jetsSortedbySpanet', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            if not (model.freeze_classifier):
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    if e % 2 == 0:
                        ...
                        fullGeneratedEvent = sample_fullRecoEvent_classifier(model, logScaled_partons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
