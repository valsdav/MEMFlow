...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['newInputDNN', 'pt_cut', 'correct_partons', 'AdamW', 'gaussian_latentSpace', 'newScaledLogReco-phi', 'classifierOnlyNonExist', 'periodicPhi', 'leptonMETFirst+newFlows+H/thad/tlep', '1Dflow-autoreg', 'onlyExistJets', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    difference_pt = 1/10*unscale_pt_cut(logScaled_partons[:,:config.transferFlow.no_max_objects, 0],
                                                        maskPartons[:,:config.transferFlow.no_max_objects],
                                                        log_mean_parton, log_std_parton, minPt_eachObj)[:,0]
                   ...
                   # sometimes there are nans if the difference_pt is too large
                    ...
                    if i == 0 and e % 2 == 0:
                        ...
                        fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian_classifier(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
