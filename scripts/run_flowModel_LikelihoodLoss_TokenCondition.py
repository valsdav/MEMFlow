...
from memflow.unfolding_flow.unfolding_flow_withDecay_v2 import UnfoldingFlow_withDecay_v2
...
def compute_regr_losses(MMD_input, MMD_target,
    ...
    loss = torch.zeros(12, device=device, dtype=dtype)
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    train_dataset = DatasetCombined(config.input_dataset_train, dev=device, new_higgs=True,
                           ...
                           reco_list_lab=['scaledLogReco_sortedBySpanet',
                                          ...
                                          'mask_boost', 'scaledLogBoost'],
                           parton_list_cm=['phasespace_intermediateParticles_onShell_logit_scaled'],
                           parton_list_lab=['tensor_AllPartons', 'mean_log_data_boost',
                                           'Hb1_thad_b_q1_tlep_b_el_CM'])
    ...
    model = UnfoldingFlow_withDecay_v2(scaling_partons_CM_ps=[mean_ps, scale_ps],
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name,config.version, 'Unfolding Flow', 'withDecay', 'angles_CM_withTheta', 'likelihood+Samples',
                      'TokenCond', f'autoreg={config.unfolding_flow.autoregressive}', f'freeze_pretrain={disable_grad_conditioning}'])
        exp.log_parameters({"model_param_tot": count_parameters(model)})
        exp.log_parameters(config)
        
        exp.set_name(f"UnfoldingFlow_withDecay_TokenCond_freeze:{disable_grad_conditioning}_{randint(0, 1000)}")
    else:
        exp = None
    ...
    No_regressed_vars = 9
    # 9 partons
    pos_partons = sinusoidal_positional_embedding(token_sequence_size=No_regressed_vars + 1, # "+1" for additional token to condition the flow
                                                  token_embedding_dim=config.conditioning_transformer.hidden_features,
                                                  device=device,
                                                  n=10000.0)
    ...
    posLogParton = torch.linspace(0, 1, No_regressed_vars+1, device=device, dtype=dtype) # +1 for new token
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            (logScaled_partons,
             ...
             #ps_detjacobian_onShell) = data_batch
            ...
            flow_prob_logit_scaled_ps, \
            flow_prob_higgs, flow_prob_thad_b, flow_prob_thad_W, \
            flow_prob_tlep_b, flow_prob_tlep_W  = ddp_model(logScaled_reco_sortedBySpanet, data_boost_reco,
                                                                    ...
                                                                    Nsamples=1, No_regressed_vars=10,
            ...
            mdmm_return = MDMM_module(flow_prob_total, [(regressed_H_b1_b2_thad_b_q1_q2_tlep_b_el_nu_ISR_ptetaphi_scaled_lab, logScaled_partons[:,[0,1,8,5,6,7,10,2,4,3,9,11]], \
                                                          #boost_regressed_Epz_scaled, logScaled_parton_boost, \
                                                          config.cartesian, loss_fn, device, dtype, False), \
                                                        *[([MMD_input_regression[particle][...,feature:feature+1]], [MMD_target[particle][...,feature:feature+1]], config.training_params.mmd_kernel, device, dtype, False, False) for feature in range(3) for particle in range(len(MMD_target)) if feature != 2 and particle != len(MMD_target)-1]])
            ...
            with torch.no_grad():
                ...
                if exp is not None and device==0 or world_size is None:
                    if i % 10 == 0:
                        ...
                        exp.log_metric('loss_flow_total', flow_prob_total, step=ii)
        ...
        valid_loss_regr_perObj = torch.zeros(12, device=device, dtype=dtype)
        valid_loss_mmd_perObj = [0. for i in range(36)]
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/UnfoldingFlow_withDecay_TokenCond_freeze:{disable_grad_conditioning}_date&Hour_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}'
