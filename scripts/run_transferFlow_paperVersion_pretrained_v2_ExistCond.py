...
from memflow.transfer_flow.transfer_flow_paper_pretrained_v2_ExistCond import TransferFlow_Paper_pretrained_v2_ExistCond
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_v2_ExistCond
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_pretrained_v2_ExistCond(no_recoVars=5, # exist + 3-mom + encoded_position
                no_partonVars=4,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'outputDNN_inConditioning', 'eta_removedFromMET', 'v2_MDMM_exist', 'no-btag', f'min6Jets_max8={easy_version}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                flow_pr = +flow_prob_pt + flow_prob_eta + flow_prob_phi
                #loss_main = loss_main - avg_flow_prob_exist
                #batch_flow_pr = batch_flow_pr + flow_prob_exist_batch
                #flow_pr = flow_pr + flow_prob_exist
                ...
                if i == 0:
                    ...
                    if e % 2 == 0:
                        ...
                        fullGeneratedEvent = sample_fullRecoEvent_classifier_v2_ExistCond(model, logScaled_partons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_v2_ExistCond_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
