...
from memflow.transfer_flow.transfer_flow_paper_AllPartons_Nobtag_autoreg import TransferFlow_Paper_AllPartons_Nobtag_autoreg
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_AllPartons_Nobtag_autoreg(no_recoVars=5, # exist + 3-mom + encoded_position,
                                 no_partonVars=6,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['No_btag', 'solve_minor_bugs', 'newScaledLogReco-phi-btag', 'paper_Impl', 'periodicPhi', 'leptonMETFirst+newFlows+H/thad/tlep', '1Dflow-autoreg', 'onlyExistJets', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
                ...
                if i == 0:
                   ...
                   # sometimes there are nans if the difference_pt is too large
                    ...
                    if i == 0 and e % 2 == 0:
                        ...
                        fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_AllPartons_Nobtag_autoreg_new_scaled_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
