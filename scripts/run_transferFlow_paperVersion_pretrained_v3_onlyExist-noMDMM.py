...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'noMDMM', '3DFlow', 'eta_removedFromMET', 'onlyExistJets', 'v2_MDMM_exist', 'no-btag', f'min6Jets_max8={easy_version}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    loss_existence = nn.BCELoss(reduce='None')    
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            loss_main_perObj = loss_existence(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])
            loss_per_batch = -flow_prob_jet_batch + torch.sum(loss_main_perObj*mask_recoParticles, dim=1)
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    wrong_pt_loss_main_perObj = loss_existence(wrongPT_flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])
                    ...
                    wrong_ptAndEta_loss_main_perObj = loss_existence(wrongPTandETA_flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])
