...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', '3DFlow', 'eta_removedFromMET', 'onlyExistJets', 'v2_MDMM_exist', 'no-btag', f'min6Jets_max8={easy_version}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.log_parameters({"model_param_flow":count_parameters(model.flow_kinematics)})
    else:
        exp = None
    ...
    loss_existence = nn.BCELoss()
    # Constraints
    constraint_existence_classifier = mdmm.MaxConstraint(
                    loss_existence,
    ...
    MDMM_module = mdmm.MDMM([constraint_existence_classifier])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            avg_flow_prob_jet, flow_prob_jet_batch, flow_prob_jet, \
            ...
            loss_main = -avg_flow_prob_jet
            flow_pr = +flow_prob_jet

            mdmm_return = MDMM_module(loss_main, [(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])])

            constrain_loss = loss_existence(flow_prob_exist, logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects,0])

            # modify mask_recoParticles: use only jets with exist==1
            mask_recoParticles = logScaled_reco_sortedBySpanet[:,:config.transferFlow.no_max_objects, 0] == 1
            ...
            with torch.no_grad():
                if exp is not None and device==0 or world_size is None:
                    if i % config.training_params.interval_logging_steps == 0:
                        ...
                        exp.log_metric('loss_step_exist', constrain_loss, step=ii)
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                loss_main = -avg_flow_prob_jet
                flow_pr = +flow_prob_jet
                batch_flow_pr = +flow_prob_jet_batch
                ...
                mask_recoParticles = logScaled_reco_sortedBySpanet[:,:,0] == 1
                ...
                if i == 0:
                    ...
                    # TODO ADD 10 % of eta
                    difference_eta = [sign_1*1/10*0.3]
                    ...
                    wrong_pt_batch_flow_pr = +wrongPT_flow_prob_jet_batch
                    wrong_pt_batch_flow_pr += wrongPT_flow_prob_exist_batch
                    ...
                    wrong_ptAndEta_batch_flow_pr = +wrongPTandETA_flow_prob_jet_batch
                    wrong_ptAndEta_batch_flow_pr += wrongPTandETA_flow_prob_exist_batch
                    ...
                    wrong_pt_batch_parton_flow_pr = +wrongPT_flow_prob_jet_batch
                    wrong_pt_batch_parton_flow_pr += wrongPT_flow_prob_exist_batch
                    ...
                    wrong_ptAndEta_batch_parton_flow_pr = +wrongPTandETA_flow_prob_jet_batch
                    wrong_ptAndEta_batch_parton_flow_pr += wrongPTandETA_flow_prob_exist_batch
                    ...
                    if e % 2 == 0:
                        ...
                        fullGeneratedEvent = sample_fullRecoEvent_classifier_v3(model, logScaled_partons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
                        ...
                        for jet in range(config.transferFlow.no_max_objects):
                            ...
                            #sampling_print(exp, fullGeneratedEvent, logScaled_reco_sortedBySpanet, mask_recoParticles, jet, e, onlyExistElem=False)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_v2_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
