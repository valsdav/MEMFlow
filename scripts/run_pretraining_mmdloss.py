    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data in enumerate(trainingLoader):
            ...
            with torch.no_grad():
                ...
                writer.add_scalar('loss_huber_H', lossH.item(), ii)
                writer.add_scalar('loss_huber_Thad', lossThad.item(), ii)
                writer.add_scalar('loss_huber_Tlep', lossTlep.item(), ii)
                writer.add_scalar('loss_huber_Gluon', lossGluon.item(), ii)
                writer.add_scalar('loss_huber_tot', (lossH+lossThad+lossTlep+lossGluon))
        ...
        for i, data in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                loss = (lossH + lossThad + lossTlep + lossGluon)/12
                print(f"validation loss:{loss.item()}")

                valid_loss_huber += loss.item()
                valid_lossH += lossH.item()
                valid_lossTlep += lossTlep.item()
                valid_lossThad += lossThad.item()
                valid_lossGluon += lossGluon.item()
        ...
        writer.add_scalar("Loss_epoch_mmd", valid_loss_mmd/N_valid, e)
