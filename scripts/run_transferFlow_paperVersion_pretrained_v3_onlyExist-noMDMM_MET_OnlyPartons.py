...
from memflow.transfer_flow.transfer_flow_paper_pretrained_v3_onlyExist_met_partons import TransferFlow_Paper_pretrained_v3_onlyExist_MET_partons
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_v3_leptonMET_partons
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    train_dataset = DatasetCombined(config.input_dataset_train, dev=device,
                           ...
                           parton_list_lab=['LogScaled_partonsLeptons',
                                           'mask_partonsLeptons'])
    ...
    # Initialize model
    model = TransferFlow_Paper_pretrained_v3_onlyExist_MET_partons(no_recoVars=5, # exist + 3-mom + encoded_position
                no_partonVars=5,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'Onlypartons', 'leptonMETFirst+newFlows+H/thad/tlep', 'noMDMM', '3DFlow', 'eta_removedFromMET', 'onlyExistJets', 'no-btag', f'min5Jets={min5Jets}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                loss_per_pt = compute_loss_per_pt(loss_per_pt, flow_pr, logScaled_reco_sortedBySpanet, mask_recoParticles_exist, log_mean_reco, log_std_reco, config.transferFlow.no_max_objects,
                        pt_bins=pt_bins)
            ...
            if i == 0 and e % 2 == 0:
                ...
                fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_v3_leptonMET_partons(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_v3_MET_OnlyPartons_scaled_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
