...
def compute_ps_regr_loss(ps_regr_scaled, ps_target_scaled, loss_fn):
    return loss_fn(ps_regr_scaled, ps_target_scaled)
...
def compute_mmd_ps_regr_loss(ps_regr_scaled, ps_target_scaled, kernel, device, dtype):
    return MMD(ps_regr_scaled, ps_target_scaled, kernel, device, dtype )
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if dtype == torch.float32:
        MDMM_module = mdmm.MDMM([constraint_regr_huber,
                                 constraint_regr_mmd,
                                 constraint_ps_regr_huber,
                                 constraint_ps_regr_mmd]).float() # support many constraints TODO: use a constraint for every particle
    else:
        MDMM_module = mdmm.MDMM([constraint_regr_huber,
                                 constraint_regr_mmd,
                                 constraint_ps_regr_huber,
