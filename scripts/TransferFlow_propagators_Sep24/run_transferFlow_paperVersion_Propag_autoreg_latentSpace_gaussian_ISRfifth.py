...
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['Propag_only', 'ISR_fifth', 'periodicPhi', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_gaussian_OnlyPropagators_ISRfifth_{randint(0, 1000)}")
    else:
        exp = None
    ...
    # new order: lepton, MET, higgs1, higgs2, ISR, tlep_b, thad1, thad2, thad3
    new_order_list = [6,7,0,1,8,5,2,3,4]
    index_elements_alwaysExist = [0,1,2,3] # lep/MET/higgs1/higgs2 always exist
    
    lastElems = [i+9 for i in range(no_recoObjs - 9)]
    new_order_list = new_order_list + lastElems
    new_order = torch.LongTensor(new_order_list)

    index_elements_nonExist = [0,3,6,7,8] + [i+9 for i in range(config.transferFlow.no_max_objects - 9)] # opposite of index_elements_alwaysExist

    ii = 0
    ii_valid = 0

    mask_partonsLeptons = torch.ones((config.training_params.batch_size_training,4), device=device, dtype=dtype)
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            avg_flow_prob_exist, flow_prob_exist_batch, flow_prob_exist = ddp_model(logScaled_reco_sortedBySpanet,
                                                                                    logScaled_partons,
                                                                                    data_boost_reco,
                                                                                    mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]],
                                                                                    mask_recoParticles,
                                                                                    mask_boost)
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                f1_score = multiclass_f1_score(prediction[:,index_elements_nonExist].flatten(), logScaled_reco_sortedBySpanet[:,index_elements_nonExist,0].flatten(), num_classes=2)
                ...
                if i == 0 and e % 2 == 0:
                    ...
                    fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian(model, logScaled_partons, mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]], logScaled_reco_sortedBySpanet.shape[0], device, dtype, index_alwaysExist=index_elements_alwaysExist, No_samples=1)
        ...
        if device == 0 or world_size is None:
            if early_stopper.early_stop(sum_valid_loss_pt/N_valid,
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_OnlyPropagators_latentSpace_gaussian_correctPartons_StopPt_ISRfifth_scaled_AdamW_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
