...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['Propag_only', 'ISR first', 'periodicPhi', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_gaussian_ptcut_OnlyPropagators_ISRfirst_{randint(0, 1000)}")
    else:
        exp = None
