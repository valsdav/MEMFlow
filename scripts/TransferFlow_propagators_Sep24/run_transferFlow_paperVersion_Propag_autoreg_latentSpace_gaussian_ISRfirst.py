...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['Propag_only', 'ISR_first', 'periodicPhi', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_gaussian_OnlyPropagators_ISRfirst_{randint(0, 1000)}")
    else:
        exp = None
    ...
    # new order: ISR, lepton, MET, tlep_b, higgs1, higgs2, thad1, thad2, thad3
    new_order_list = [8,6,7,5,0,1,2,3,4]
    index_elements_alwaysExist = [1,2,4,5] # lep/MET/higgs1/higgs2 always exist
