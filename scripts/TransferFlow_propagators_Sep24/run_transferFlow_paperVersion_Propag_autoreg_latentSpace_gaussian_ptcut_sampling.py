...
def train(device, path_to_dir, path_to_model, config, dtype, validation,
    ...
    no_recoObjs = test_dataset.reco_lab.scaledLogReco_sortedBySpanet_ptcut.shape[1]

    log_mean_reco = test_dataset.reco_lab.meanRecoParticles_ptcut
    ...
    No_eventsSampled = test_dataset.reco_lab.scaledLogReco_sortedBySpanet_ptcut.shape[0] / batch_size
    ...
    for i, data_batch in enumerate(testLoader):
        ...
        with torch.no_grad():
            ...
            fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian(model, logScaled_partons, mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]], logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=No_samples)
            ...
            # without phi
            fullGeneratedEvent[...,[1,2]] = (fullGeneratedEvent[...,[1,2]]*log_std_reco[:2] + log_mean_reco[:2])
