...
from memflow.transfer_flow.transfer_flow_paper_AllPartons_Nobtag_autoreg_latentSpace_gaussian_classifier import TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace_gaussian_classifier
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace_gaussian_classifier(no_recoVars=5, # exist + 3-mom + encoded_position,
                                 no_partonVars=4,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['Propag_only', 'periodicPhi', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_gaussian_ptcut_classifier_OnlyPropagators_{randint(0, 1000)}")
    else:
        exp = None
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0 and e % 2 == 0:
                    ...
                    fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian_classifier(model, logScaled_partons, mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]], logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
