...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.set_name(f"TransferFlow_gaussian_OnlyPropagators_{randint(0, 1000)}")
    else:
        exp = None
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0 and e % 2 == 0:
                    ...
                    fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian(model, logScaled_partons, mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]], logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
