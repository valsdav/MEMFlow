...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['Propag_only', 'ISR fifth', 'periodicPhi', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
        ...
        exp.set_name(f"TransferFlow_gaussian_ptcut_OnlyPropagators_ISRfifth_{randint(0, 1000)}")
    else:
        exp = None
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            loss_main_perObj[:,index_elements_alwaysExist] = 0 # always exist
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                if i == 0 and e % 2 == 0:
                    ...
                    fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_Nobtag_autoreg_latentSpace_gaussian(model, logScaled_partons, mask_partonsLeptons[:logScaled_reco_sortedBySpanet.shape[0]], logScaled_reco_sortedBySpanet.shape[0], device, dtype, index_elements_alwaysExist, No_samples=1)
