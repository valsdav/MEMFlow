...
def sample_next_token_classifier_v3_leptonMET_partons(model, logScaled_reco_sortedBySpanet, logScaled_partons, mask_reco, mask_partons, step, No_samples=1):
...
def sample_next_token_classifier_v3_leptonMET_partons_btag(model, logScaled_reco_sortedBySpanet, logScaled_partons, mask_reco, mask_partons, step, No_samples=1):
    ...
    if step == 0:
        jetsPt_Eta_phi_sampled = model.flow_kinematics_lepton(conditioning).sample((1,))
        lepton_btag = torch.zeros(jetsPt_Eta_phi_sampled[...,0:1].shape, device=model.device, dtype=model.dtype)
        jetsPt_Eta_phi_sampled = torch.cat((jetsPt_Eta_phi_sampled, lepton_btag), dim=3)
    elif step == 1:
        jetsPt_Eta_phi_sampled = model.flow_kinematics_MET(conditioning).sample((1,))
        MET_pt = jetsPt_Eta_phi_sampled[..., 0].unsqueeze(dim=3)
        MET_eta = torch.zeros(MET_pt.shape, device=model.device, dtype=model.dtype)
        MET_phi = jetsPt_Eta_phi_sampled[..., 1].unsqueeze(dim=3)
        MET_btag = torch.zeros(MET_pt.shape, device=model.device, dtype=model.dtype)
        jetsPt_Eta_phi_sampled = torch.cat((MET_pt, MET_eta, MET_phi, MET_btag), dim=3)
    else:
        jetsPt_Eta_phi_sampled = model.flow_kinematics_jets(conditioning).sample((1,))
...
def sample_next_token_classifier_AllPartons_btag(model, logScaled_reco_sortedBySpanet, logScaled_partons, mask_reco, mask_partons, step, No_samples=1):
    ...
    if step == 0:
        ...
        jetsPt_Eta_phi_sampled = model.flow_kinematics_jets(conditioning).sample((1,))
        btag_sampled = model.flow_btag(conditioning).sample((1,))
        jetsPt_Eta_phi_sampled = torch.cat((jetsPt_Eta_phi_sampled, btag_sampled), dim=3)
...
def sample_fullRecoEvent_classifier_v3_leptonMET_partons(model, logScaled_partons, mask_partons, no_events, device, dtype, No_samples=1):
    ...
    for j in range(model.no_max_objects):
        ...
        next_jet = sample_next_token_classifier_v3_leptonMET_partons(model, fullGeneratedEvent, logScaled_partons, mask_reco, mask_partons, j, No_samples)
...
def sample_fullRecoEvent_classifier_v3_leptonMET_partons_btag(model, logScaled_partons, mask_partons, no_events, device, dtype, No_samples=1):
    ...
    for j in range(model.no_max_objects):
        ...
        next_jet = sample_next_token_classifier_v3_leptonMET_partons_btag(model, fullGeneratedEvent, logScaled_partons, mask_reco, mask_partons, j, No_samples)
...
def sample_fullRecoEvent_classifier_AllPartons_btag(model, logScaled_partons, mask_partons, no_events, device, dtype, No_samples=1):
    ...
    for j in range(model.no_max_objects):
        ...
        next_jet = sample_next_token_classifier_AllPartons_btag(model, fullGeneratedEvent, logScaled_partons, mask_reco, mask_partons, j, No_samples)
...
def sample_next_token_unfoldingFlow(model, generated_partons, logScaled_jets, mask_jets, step, No_samples=1):
    ...
    # create null token and its mask
    null_token = torch.ones((generated_partons.shape[0], 1, 5), device=model.device, dtype=model.dtype) * -1

    # attach null token and update the mask for the scaling_reco_lab
    scaling_parton_lab_withNullToken = torch.cat((null_token, generated_partons), dim=1)
    
    scaledLogReco_afterLin = model.gelu(model.linearDNN_reco(logScaled_jets) * mask_jets[..., None])
    scaledLogParton_afterLin = model.gelu(model.linearDNN_parton(scaling_parton_lab_withNullToken))

    tgt_mask = model.transformer_model.generate_square_subsequent_mask(scaledLogParton_afterLin.shape[1], device=model.device)
    ...
    output_decoder = model.transformer_model(scaledLogReco_afterLin, scaledLogParton_afterLin, tgt_mask=tgt_mask)

    if model.encode_position:
        hot_encoded = model.hot_encoded.expand(output_decoder.shape[0], -1, -1)
        output_decoder = torch.cat((output_decoder, hot_encoded[:,:output_decoder.shape[1]]), dim=2)

    # take the last conditioning (the one on jet_0 ... jet_-1) 
    conditioning = output_decoder[:,-1:]

    jetsPt_Eta_phi_sampled = model.flow_kinematics_partons(conditioning).sample((1,))
    
    jetsPt_Eta_phi_sampled = jetsPt_Eta_phi_sampled.squeeze(dim=0)

    # get the first dimension of 'logScaled_reco_sortedBySpanet'
    # [1:2] to save it as a tensor
    type_position = torch.tensor([1,step], device=model.device)
    type_position = type_position.expand(output_decoder.shape[0], 1, 2)
    if type_position[0,0,1] > 6:
        type_position[:,0,0] = 2

    generated_parton = torch.cat((jetsPt_Eta_phi_sampled, type_position), dim=2)

    return generated_parton

def sample_unfoldingFlow(model, logScaled_jets, mask_jets, no_events, device, dtype, No_samples=1):
    
    fullGeneratedEvent = torch.empty((no_events*No_samples, 0, 5), device=device, dtype=dtype)
    mask_one = torch.ones((no_events*No_samples, 1), device=device, dtype=dtype)
    logScaled_jets = logScaled_jets.repeat(No_samples, 1, 1)
    
    for j in range(9):

        next_parton = sample_next_token_unfoldingFlow(model, fullGeneratedEvent, logScaled_jets, mask_jets, j, No_samples)
    
        fullGeneratedEvent = torch.cat((fullGeneratedEvent, next_parton), dim=1) 
...
def sampling_print_btag(experiment, sampledEvent, logScaled_reco_target, mask_recoParticles, plotJets, epoch, onlyExistElem=False):

    # plot [pt,eta,phi]
    var_name = ['pt', 'eta', 'phi', 'btag']
...
def sampling_print_partons(experiment, sampledEvent, logScaled_partons, plotJets, epoch, onlyExistElem=False):
    ...
    for plot_var in range(3):
        ...
        fig, ax = plt.subplots(figsize=(7,6), dpi=100)
        diff_generatedAndTarget = (sampledEvent[:,plotJets,plot_var] - logScaled_partons[:,plotJets,plot_var])
        ax.hist(diff_generatedAndTarget.detach().cpu().numpy(), range=(-5,5), bins=20, histtype='step', color='b', stacked=False, fill=False)
        ax.set_xlabel(f'{var_name[plot_var]}_generated - {var_name[plot_var]}_target')
        experiment.log_figure(f"Diff_generated_{var_name[plot_var]}_exist={onlyExistElem} for partons:{plotJets}", fig, step=epoch)

        fig, ax = plt.subplots(figsize=(7,6), dpi=100)
        h = ax.hist2d(sampledEvent[:,plotJets,plot_var].detach().cpu().numpy(),
                      logScaled_partons[:,plotJets,plot_var].detach().cpu().numpy(),
                      bins=30, range=[(-5,5),(-5,5)], cmin=1)
        fig.colorbar(h[3], ax=ax)
        ax.set_xlabel(f'sampled {var_name[plot_var]}')
        ax.set_ylabel(f'target {var_name[plot_var]}')
        experiment.log_figure(f"2D_correlation_{var_name[plot_var]}_exist={onlyExistElem} for partons:{plotJets}", fig, step=epoch)
...
def unscale_pt_boxcox(pt_reco, mask_recoParticles, log_mean_reco, log_std_reco, param_lambda):
    # pt is on the 2nd position, exist flag is on the first position (only for logScaled_reco)
    unscaled_pt = pt_reco*log_std_reco[0] + log_mean_reco[0]
    unscaled_pt = torch.pow(param_lambda*unscaled_pt + 1, 1/param_lambda)
    unscaled_pt = unscaled_pt*mask_recoParticles # set masked objects to 0
    return unscaled_pt

def compute_loss_per_pt_boxcox(loss_per_pt, flow_pr, scaledLogReco, maskedReco, log_mean_reco, log_std_reco, no_max_objects, param_lambda,
                        pt_bins=[5, 50, 75, 100, 150, 200, 300, 1500, 3000]):
    unscaled_pt = unscale_pt_boxcox(scaledLogReco[:,:no_max_objects,1], maskedReco[:,:no_max_objects], log_mean_reco, log_std_reco, param_lambda)
...
def compute_loss_per_pt_partons(loss_per_pt, flow_pr, scaledLogReco, maskedReco, log_mean_reco, log_std_reco,
                        pt_bins=[5, 50, 75, 100, 150, 200, 300, 1500, 3000]):
    unscaled_pt = unscale_pt(scaledLogReco, maskedReco, log_mean_reco, log_std_reco, 9)
