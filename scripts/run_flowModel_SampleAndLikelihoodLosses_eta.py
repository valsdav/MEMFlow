...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name,config.version, 'Unfolding Flow', 'eta', 'likelihood+Samples',
        ...
        exp.set_name(f"UnfoldingFlow_withDecay_SampleLikelihoodLosses_eta_freeze:{disable_grad_conditioning}_{randint(0, 1000)}")
    else:
        exp = None
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/UnfoldingFlow_withDecay_SampleLikelihoodLosses_eta_freeze:{disable_grad_conditioning}_date&Hour_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}'
