...
def loss_fn_periodic(inp, target, loss_fn, device):
    ...
    # rescale to pi
    # overflow_delta = (torch.clamp(inp - PI, min=0.) - torch.clamp(inp + PI, max=-PI)).mean()/3
...
def TrainingAndValidLoop(config, device, model, trainingLoader, validLoader, outputDir, HuberLoss,
    ...
    for e in range(config.training_params.nepochs):
        ...
        model_val = model.module # non parallel model
        model_val.eval()
        ...
        for i, data in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                out = model_val(logScaled_reco, data_boost_reco, mask_recoParticles, mask_boost_reco)
...
if __name__ == '__main__':
    ...
    val_loader = DataLoader(dataset=val_subset, shuffle=False, batch_size=conf.training_params.batch_size_validation)
