...
from memflow.transfer_flow.transfer_flow_paper_pretrained_v2 import TransferFlow_Paper_pretrained_v2
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_pretrained_v2(no_recoVars=5, # exist + 3-mom + encoded_position
                no_partonVars=4,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'eta_removedFromTraining', 'v2_MDMM_exist', 'no-btag', f'min6Jets_max8={easy_version}', 'only_exist_pt_eta_phi', 'jetsSortedbySpanet', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
