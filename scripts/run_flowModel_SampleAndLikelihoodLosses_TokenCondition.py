...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name,config.version, 'token cond', 'Unfolding Flow', 'Sample Loss', 'withDecay', 'angles_CM_withTheta', 'likelihood+Samples',
        ...
        exp.set_name(f"UnfoldingFlow_withDecay_SampleLoss_TokenCond_freeze:{disable_grad_conditioning}_{randint(0, 1000)}")
    else:
        exp = None
    ...
    No_regressed_vars = 9
    # 9 partons
    pos_partons = sinusoidal_positional_embedding(token_sequence_size=No_regressed_vars + 1, # new token
                                                  token_embedding_dim=config.conditioning_transformer.hidden_features,
                                                  device=device,
                                                  n=10000.0)
    ...
    # keep the same previous tokens
    posLogParton = torch.linspace(0, 1.1111, No_regressed_vars+1, device=device, dtype=dtype) # modify to make sense for new token
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            sampled_Hb1b2_thad_b_q1q2_tlep_b_el_nu_ISR_Eptetaphi_scaled_lab, boost_sampled_scaled  = ddp_model(logScaled_reco_sortedBySpanet, data_boost_reco,
                                                                    ...
                                                                    flow_eval="both",
                                                                    Nsamples=1, No_regressed_vars=No_regressed_vars + 1,
            ...
            mdmm_return = MDMM_module(full_loss, [(regressed_H_b1_b2_thad_b_q1_q2_tlep_b_el_nu_ISR_ptetaphi_scaled_lab, logScaled_partons[:,[0,1,8,5,6,7,10,2,4,3,9,11]], \
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/UnfoldingFlow_withDecay_SampleLoss_TokenCond_freeze:{disable_grad_conditioning}_date&Hour_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}'
