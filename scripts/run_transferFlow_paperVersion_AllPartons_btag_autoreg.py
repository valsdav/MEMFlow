...
from memflow.transfer_flow.transfer_flow_paper_AllPartons_btag_autoreg import TransferFlow_Paper_AllPartons_btag_autoreg
from memflow.unfolding_flow.utils import *
from utils_transferFlow_paper import sample_fullRecoEvent_classifier_AllPartons_btag_autoreg
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_AllPartons_btag_autoreg(no_recoVars=6, # exist + 3-mom + encoded_position + btag,
                                 no_partonVars=6,
                                 ...
                                 flow_nfeatures=1, #pt/eta/phi/btag(logit)
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['solve_minor_bugs', 'newScaledLogReco-phi-btag', 'paper_Impl', 'btag-logit-NotStandardized', 'periodicPhi', 'leptonMETFirst+newFlows+H/thad/tlep', '1Dflow-autoreg', 'onlyExistJets', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        sum_loss_pt = sum_loss_eta = sum_loss_phi = sum_loss_exist = sum_loss_btag = 0.
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            avg_flow_prob_btag, flow_prob_btag_batch, flow_prob_btag, \
            ...
            flow_pr = +flow_prob_pt + flow_prob_eta + flow_prob_phi + flow_prob_btag + loss_main_perObj
            ...
            if i == 0 and e % 2 == 0:
                ...
                plot_grad_flow(exp, named_parameters=ddp_model.flow_btag.named_parameters(),
                               epoch=e, modelName='flow_btag')
                ...
                plot_grad_hist(exp, named_parameters=ddp_model.flow_kinematics_phi.named_parameters(),
                           epoch=e, modelName='flow_phi')
                plot_grad_hist(exp, named_parameters=ddp_model.flow_btag.named_parameters(),
                           epoch=e, modelName='flow_btag')
            ...
            sum_loss_btag += -avg_flow_prob_btag
            ...
            with torch.no_grad():
                if exp is not None and device==0 or world_size is None:
                    if i % config.training_params.interval_logging_steps == 0:
                        ...
                        exp.log_metric('loss_step_btag', -avg_flow_prob_btag, step=ii)
        ...
        if exp is not None and device==0 or world_size is None:
            ...
            exp.log_metric("loss_train_epoch_btag", sum_loss_btag/N_train, epoch=e)
        ...
        sum_valid_loss_pt = sum_valid_loss_eta = sum_valid_loss_phi = sum_valid_loss_exist = sum_valid_loss_btag = 0.
        f1_valid_total = 0.
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                sum_valid_loss_btag += -avg_flow_prob_btag
                ...
                if i == 0:
                    ...
                    wrongPT_avg_flow_prob_btag, wrongPT_flow_prob_phi_btag, wrongPT_flow_prob_btag, \
                    ...
                    wrongPTandETA_avg_flow_prob_btag, wrongPTandETA_flow_prob_phi_btag, wrongPTandETA_flow_prob_btag, \
                   ...
                   # sometimes there are nans if the difference_pt is too large
                    ...
                    if i == 0 and e % 2 == 0:
                        ...
                        fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_AllPartons_btag_autoreg(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
        ...
        if exp is not None and device==0 or world_size is None:
            ...
            exp.log_metric("total_valid_loss_btag", sum_valid_loss_btag/N_valid, epoch=e)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_AllPartons_btag_autoreg_new_scaled_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
