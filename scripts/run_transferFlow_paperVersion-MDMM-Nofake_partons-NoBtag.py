...
from utils_transferFlow_paper import sample_next_token
...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name, config.version, 'paper Implementation', 'no-btag', 'no-fake-permutation', 'only_exist_pt_eta_phi', 'jetsSortedbySpanet', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}', f'min6Jets_max8={easy_version}'])
    ...
    loss_similarity = nn.CosineEmbeddingLoss()
    # Constraints
    constraint_similarity_next_token = mdmm.MaxConstraint(
                    loss_similarity,
    ...
    MDMM_module = mdmm.MDMM([constraint_similarity_next_token])
    ...
    for e in range(config.training_params.nepochs):
        ...
        sum_loss = 0.
        sum_loss_constrain_similarity = 0.
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            mdmm_return = MDMM_module(loss_main, [(next_token, logScaled_reco_sortedBySpanet[:,jet_iteration], torch.ones(next_token.shape[0], device=device, dtype=dtype))])

            constrain_loss = loss_similarity(next_token, logScaled_reco_sortedBySpanet[:,jet_iteration], torch.ones(next_token.shape[0], device=device, dtype=dtype))
            ...
            sum_loss_constrain_similarity += constrain_loss.item()
            ...
            with torch.no_grad():
                if exp is not None and device==0 or world_size is None:
                    if i % config.training_params.interval_logging_steps == 0:
                        ...
                        exp.log_metric('loss_step_constrainSimilarity', constrain_loss, step=ii)
        ...
        if exp is not None and device==0 or world_size is None:
            ...
            exp.log_metric("loss_train_epoch_constrainSimilarity", sum_loss_constrain_similarity/N_train, epoch=e)
        ...
        total_valid_loss = 0.
        total_valid_loss_constrainSimilarity = 0.
        ...
        for i, data_batch in enumerate(validLoader):
            ...
            with torch.no_grad():
                ...
                total_valid_loss_constrainSimilarity += constrain_loss.item()
        ...
        if exp is not None and device==0 or world_size is None:
            ...
            exp.log_metric("total_valid_loss_constrainSimilarity", total_valid_loss_constrainSimilarity/N_valid, epoch=e)
