...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags([config.name,config.version, 'combineDecayAndPropag', 'phi optimized', '1D MMD constraints', 'angles_CM', 'moreMMD', 'optimizeMMD'])
    ...
    for e in range(config.training_params.nepochs):
        ...
        for i, data_batch in enumerate(trainingLoader):
            ...
            propagators_momenta, decayVars_momenta, boost_regressed = ddp_model(logScaled_reco_sortedBySpanet,
            ...
            H_b1_b2_thad_b_q1_q2_tlep_b_el_nu_ISR = Compute_ParticlesTensor.get_decayPartons_fromlab_propagators_angles(propagators_momenta,
                                                      ...
                                                      return_both=False, pt_cut=None, unscale_phi=False, debug=False)
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/preTraining_condTransformer_combineDecayAndPropag_anglesCM__1dMMDconstraints_phiOptimized_date&Hour_{datetime.now().strftime("%d/%m/%Y_%H:%M:%S")}'
