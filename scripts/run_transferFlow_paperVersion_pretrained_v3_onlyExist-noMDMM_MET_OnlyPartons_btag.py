...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_AllPartons_btag(no_recoVars=6, # exist + 3-mom + logit(btag) + encoded_position 
                no_partonVars=6, # [pt,eta,phi,decay/PDG,parton_type,order]
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['all-partons', '6-parton-vars', 'btag-logit-NotStandard', 'leptonMETFirst+newFlows+H/thad/tlep', '3DFlow', 'eta_removedFromMET', 'onlyExistJets', 'no-btag', f'min5Jets={min5Jets}', 'HiggsAssignment', f'scheduler={config.training_params.scheduler}'])
...
if __name__ == '__main__':
    ...
    name_dir = f'{outputDir}/Transfer_Flow_Paper_pretrained_v3_MET_OnlyPartons_btag_scaled_min5Jets={min5Jets}_{conf.name}_{conf.version}_{conf.transferFlow.base}_NoTransf{conf.transferFlow.ntransforms}_NoBins{conf.transferFlow.bins}_DNN:{conf.transferFlow.hiddenMLP_NoLayers}_{conf.transferFlow.hiddenMLP_LayerDim}'
