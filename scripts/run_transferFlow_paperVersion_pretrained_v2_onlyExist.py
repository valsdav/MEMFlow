...
def train( device, name_dir, config,  outputDir, dtype,
    ...
    # Initialize model
    model = TransferFlow_Paper_pretrained_v2_onlyExist(no_recoVars=5, # exist + 3-mom + encoded_position
                no_partonVars=4,
    ...
    if device == 0 or world_size is None:
        ...
        exp.add_tags(['paper_Impl', 'eta_removedFromMET', 'onlyExistJets', 'v2_MDMM_exist', 'no-btag', f'min6Jets_max8={easy_version}', 'HiggsAssignment', 'null_token_only_in_transformer', f'scheduler={config.training_params.scheduler}'])
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    if e % 2 == 0:
                        ...
                        fullGeneratedEvent = sample_fullRecoEvent_classifier_v2(model, logScaled_partons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=1)
