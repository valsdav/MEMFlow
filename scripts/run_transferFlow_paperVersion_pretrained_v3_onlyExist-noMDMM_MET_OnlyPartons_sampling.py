...
def train(device, path_to_dir, path_to_model, config, dtype, old_order, validation,
    ...
    for i, data_batch in enumerate(testLoader):
        ...
        with torch.no_grad():
            ...
            fullGeneratedEvent, mask_reco = sample_fullRecoEvent_classifier_v3_leptonMET_partons(model, logScaled_partons, mask_partonsLeptons, logScaled_reco_sortedBySpanet.shape[0], device, dtype, No_samples=No_samples)
