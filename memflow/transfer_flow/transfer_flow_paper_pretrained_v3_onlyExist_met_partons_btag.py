...
class TransferFlow_Paper_pretrained_v3_onlyExist_MET_partons_btag(nn.Module):
    def __init__(self,
        ...
        super(TransferFlow_Paper_pretrained_v3_onlyExist_MET_partons_btag, self).__init__()
        ...
        # flow for jets
        self.flow_kinematics_jets = zuko.flows.NSF(features=4,
                              context=transformer_input_features + 1, # additional position of 'jet'
    ...
    def forward(self, scaling_reco_lab, scaling_partons_lab, scaling_RegressedBoost_lab,
        ...
        flow_prob_jet = self.flow_kinematics_jets(conditioning[:,2:]).log_prob(scaling_reco_lab[:,2:self.no_max_objects,1:5])

        # compute flow prob per event
        flow_prob_batch = torch.sum(flow_prob_jet*maskExist, dim=1) + flow_prob_lepton + flow_prob_MET # take avg of masked objects
        ...
        return avg_flow_prob, flow_prob_batch, flow_prob_jet, flow_prob_MET, flow_prob_lepton, \
