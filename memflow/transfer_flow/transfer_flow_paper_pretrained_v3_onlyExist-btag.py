...
class TransferFlow_Paper_pretrained_v3_onlyExist_btag(nn.Module):
    def __init__(self,
        ...
        super(TransferFlow_Paper_pretrained_v3_onlyExist_btag, self).__init__()
    ...
    def forward(self, scaling_reco_lab, scaling_partons_lab, scaling_RegressedBoost_lab,
        ...
        # very important: pt on the 2nd position (pt/eta/phi/btag)
        flow_prob_jet = self.flow_kinematics(conditioning).log_prob(scaling_reco_lab[:,:self.no_max_objects,0:4])
        flow_prob_jet_batch = torch.sum(flow_prob_jet*mask_reco, dim=1) # take avg of masked objects
        ...
        prob_each_event = torch.sum(prob_each_jet, dim=1) # take avg of masked objects
