...
class TransferFlow_Paper_pretrained(nn.Module):
    def __init__(self,
        ...
        super(TransferFlow_Paper_pretrained, self).__init__()
        ...
        self.flow_eta = zuko.flows.NSF(features=flow_nfeatures,
                              context=transformer_input_features + 2, # additional position of 'jet' + condition pt
                              transforms=flow_ntransforms, 
                              bins=flow_bins, 
                              hidden_features=[flow_hiddenMLP_LayerDim]*flow_hiddenMLP_NoLayers, 
                              randperm=randPerm,
        ...
        self.flow_phi = zuko.flows.NSF(features=flow_nfeatures,
                              context=transformer_input_features + 3, # additional position of 'jet' + condition pt/eta
                              transforms=flow_ntransforms, 
                              bins=flow_bins, 
                              hidden_features=[flow_hiddenMLP_LayerDim]*flow_hiddenMLP_NoLayers, 
                              randperm=randPerm,
    ...
    def forward(self, scaling_reco_lab, scaling_partons_lab, scaling_RegressedBoost_lab,
        ...
        prob_each_jet = prob_each_event = prob_avg = 0

        if not (self.freeze_classifier):   
            output_decoder_classifier = self.classifier_exist.transformer_model(scaledLogParton_afterLin, scaledLogReco_afterLin, tgt_mask=self.classifier_exist.tgt_mask)
            if self.encode_position:
                hot_encoded = self.classifier_exist.hot_encoded.expand(output_decoder_classifier.shape[0], -1, -1)
                output_decoder_classifier = torch.cat((output_decoder_classifier, hot_encoded), dim=2)
                
            prob_each_jet = self.classifier_exist.model(output_decoder_classifier).squeeze(dim=2)
