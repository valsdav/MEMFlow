...
class TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace(nn.Module):
    def __init__(self,
        ...
        super(TransferFlow_Paper_AllPartons_Nobtag_autoreg_latentSpace, self).__init__()
    ...
    def forward(self, scaling_reco_lab, scaling_partons_lab, scaling_RegressedBoost_lab,
        ...
        # FLOW ETA
        pt_latentSpace = self.flow_kinematics_pt(conditioning_pt).transform(scaled_reco_lab_pt)
        conditioning_eta = torch.cat((output_decoder[:,:self.no_max_objects], scaled_reco_lab_phi, pt_latentSpace), dim=2)
        ...
        # FLOW PHI
        eta_latentSpace = self.flow_kinematics_eta(conditioning_eta).transform(scaled_reco_lab_eta)
        conditioning_phi = torch.cat((output_decoder[:,:self.no_max_objects], pt_latentSpace, eta_latentSpace), dim=2)
