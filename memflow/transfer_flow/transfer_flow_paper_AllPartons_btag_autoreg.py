...
class TransferFlow_Paper_AllPartons_btag_autoreg(nn.Module):
    def __init__(self,
        ...
        super(TransferFlow_Paper_AllPartons_btag_autoreg, self).__init__()
        ...
        self.flow_btag = zuko.flows.NSF(features=1,
                              context=transformer_input_features + 4, # additional position of 'jet'/phi/eta/pt
                              transforms=flow_ntransforms, 
                              bins=flow_bins, 
                              hidden_features=[flow_hiddenMLP_LayerDim]*flow_hiddenMLP_NoLayers, 
                              randperm=randPerm,
                              #base=eval(flow_base_btag),
                              #base_args=[torch.ones(1)*flow_base_first_arg, torch.ones(1)*flow_base_first_arg],
                              #dtype=dtype,
                              #univariate_kwargs={"bound": 8 }, # Keeping the flow in the [-B,B] box.
                              passes= 2 if not flow_autoregressive else 1)
        ...
        if dtype == torch.float32:
            ...
            self.flow_btag = self.flow_btag.float()
            ...
            self.flow_btag = self.flow_btag.double()
    ...
    def forward(self, scaling_reco_lab, scaling_partons_lab, scaling_RegressedBoost_lab,
        ...
        # FLOW btag
        conditioning_btag = torch.cat((output_decoder[:,:self.no_max_objects], scaled_reco_lab_phi, scaled_reco_lab_eta, scaled_reco_lab_pt), dim=2) # add phi/eta/pt in conditioning
        # very important: pt on the 2nd position
        scaled_reco_lab_btag = scaling_reco_lab[:,:self.no_max_objects,4].unsqueeze(dim=2)
        flow_prob_btag = self.flow_btag(conditioning_btag).log_prob(scaled_reco_lab_btag)
        flow_prob_btag[:,0] = 0 # set log_prob(btag) for lepton = 0 -> doesn't affect computations
        flow_prob_btag[:,1] = 0 # set log_prob(btag) for MET = 0 -> doesn't affect computations
        flow_prob_btag_batch = torch.sum(flow_prob_btag*maskExist[:,:self.no_max_objects], dim=1) # take avg of masked objects
        avg_flow_prob_btag = flow_prob_btag_batch.mean()

        # compute flow prob per event
        flow_prob_batch = flow_prob_phi_batch + flow_prob_eta_batch + flow_prob_pt_batch + flow_prob_btag_batch # take avg of masked objects
